...
/**
 * mpi3mr_repost_reply_buf - Post reply buffer to queue
 * @mrioc: Adapter instance reference
...
/**
 * mpi3mr_repost_sense_buf - Post sense buffer to queue
 * @mrioc: Adapter instance reference
 ...
 * host tag is un identifiable then also NULL is returned.
 *
	...
	struct op_reply_qinfo *op_reply_q)
{
	...
	/*Not to be ported to upstream*/
#ifndef CONFIG_PREEMPT_RT
		if (!in_task() && (threaded_isr_poll == true) &&
		    (mrioc->poll_completions ||
		     scsi_host_in_recovery(mrioc->shost))) {
...
void mpi3mr_print_fault_info(struct mpi3mr_ioc *mrioc)
{
	...
	if (ioc_status & MPI3_SYSIF_IOC_STATUS_FAULT) {
		...
		ioc_info(mrioc,
		    "fault code(0x%04X): additional code: (0x%08X:0x%08X:0x%08X)\n",
...
static void mpi3mr_free_ioctl_dma_memory(struct mpi3mr_ioc *mrioc)
{
	...
	for (i = 0; i < MPI3MR_NUM_IOCTL_SGE; i++) {
		...
		if (mem_desc->addr) {
			dma_pool_free(mrioc->ioctl_dma_pool, mem_desc->addr,
			    mem_desc->dma_addr);
			mem_desc->addr = NULL;
		}
	}
...
static void mpi3mr_alloc_ioctl_dma_memory(struct mpi3mr_ioc *mrioc)
{
	...
	mrioc->ioctl_dma_pool = dma_pool_create("ioctl dma pool",
	    &mrioc->pdev->dev, MPI3MR_IOCTL_SGE_SIZE, MPI3MR_PAGE_SIZE_4K, 0);
	if (!mrioc->ioctl_dma_pool) {
    ...
    u16 reset_reason)
{
	...
	ioc_info(mrioc, "ioc_status/ioc_config after %s message unit reset is (0x%08x)/(0x%08x)\n",
    ...
    u16 reset_reason)
{
	...
	do {
		...
		for (i = 0; i < 5; i++) {
			...
			if (host_diagnostic & MPI3_SYSIF_HOST_DIAG_DIAG_WRITE_ENABLE)
				break;
			udelay(200);
		}
	...
	u16 admin_req_sz, u8 ignore_reset)
{
	...
	{
		dprint_init(mrioc, "admin request queue submission failed due to controller not in ready state\n");
		retval = -EAGAIN;
		goto out;
	}
	...
	if (mrioc->logging_level & MPI3_DEBUG_ADMIN_REQ)
		dprint_dump(admin_req,
		    admin_req_sz, "posting mpi3_admin_req:");
	...
	struct op_req_qinfo *op_req_q, u8 *req)
{
	...
	if (mrioc->reset_in_progress) {
		ioc_err(mrioc, "operational request queue submission failed due to reset in progress\n");
		retval = -EAGAIN;
		goto out;
	}
	if (mrioc->pcie_err_recovery) {
	...
	if ((atomic_inc_return(&mrioc->op_reply_qinfo[reply_qidx].pend_ios)
	    > MPI3MR_IRQ_POLL_TRIGGER_IOCOUNT) && (threaded_isr_poll == true))
...
static int mpi3mr_sync_timestamp(struct mpi3mr_ioc *mrioc)
{
	...
	retval = mpi3mr_admin_request_post(mrioc, &iou_ctrl,
	    sizeof(iou_ctrl), 0);
	if (retval) {
		ioc_err(mrioc, "posting time stamp update failed\n");
		goto out_unlock;
	}
	...
	ioc_status = mrioc->init_cmds.ioc_status & MPI3_IOCSTATUS_STATUS_MASK;
...
mpi3mr_upload_drv_diag_buffer(struct mpi3mr_ioc *mrioc)
{
	...
	if (!mrioc->facts.diag_drvr_sz) {
		dprint_init(mrioc, "driver diag buffer upload is not needed\n");
...
static void mpi3mr_watchdog_work(struct work_struct *work)
{
	...
	if (atomic_read(&mrioc->admin_pend_isr)) {
                ioc_err(mrioc, "Unprocessed admin ISR instance found, flush admin replies\n");
                mpi3mr_process_admin_reply_q(mrioc);
	}
	...
	if (host_diagnostic & MPI3_SYSIF_HOST_DIAG_SAVE_IN_PROGRESS) {
		...
		if ((mrioc->diagsave_timeout++) <= ds_timeout)
			goto schedule_work;
	}
	...
	struct mpi3_ioc_facts_data *facts_data)
{
	...
	if (mrioc->facts.io_throttle_data_length)
		...
		/* set the length to max len + 1K to disable throttle*/
		mrioc->io_throttle_data_length =
		    (mrioc->facts.max_data_length / 512) + 2;
	...
	ioc_info(mrioc, "dma_mask(%d), initial_port_enable_status(0x%02x), max_data_len (%d)\n",
	    mrioc->facts.dma_mask, (facts_flags &
	    MPI3_IOCFACTS_FLAGS_INITIAL_PORT_ENABLE_MASK),
	    mrioc->facts.max_data_length);
	ioc_info(mrioc,
	    "diag_trace_size(%dKB), diag_fw_size(%dKB), diag_drvr_size(%dKB)\n",
...
static int mpi3mr_issue_iocinit(struct mpi3mr_ioc *mrioc)
{
	...
	drv_info->driver_capabilities =
		MPI3_IOCINIT_DRIVERCAP_OSEXPOSURE_NO_SPECIAL;
...
static int mpi3mr_alloc_chain_bufs(struct mpi3mr_ioc *mrioc)
{
	...
	dprint_init(mrioc, "number of sgl entries=%d chain buffer size=%dKB\n",
...
static int mpi3mr_bring_ioc_ready(struct mpi3mr_ioc *mrioc)
{
	...
	u64 base_info;
	u8 retry = 0;
	u64 start_time = 0, elapsed_time_sec;
...
retry_init:
	...
	if (!mrioc->throttle_groups && mrioc->num_io_throttle_group) {
		...
		mrioc->throttle_groups = kcalloc(mrioc->num_io_throttle_group,
						 sz, GFP_KERNEL);
...
static void mpi3mr_issue_ioc_shutdown(struct mpi3mr_ioc *mrioc)
{
	...
	ioc_info(mrioc,
	    "ioc_status/ioc_config after %s shutdown is (0x%08x)/(0x%08x)\n",
	    (!retval)?"successful":"failed", ioc_status,
	    ioc_config);
}
