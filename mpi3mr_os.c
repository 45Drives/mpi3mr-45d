...
/**
 * mpi3mr_fwevt_free - firmware event memory deallocator
 ...
 * . Called post reset prior to re initializing the controller.
 *
 * Return: Nothing.
 */
 ...
 */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 10, 0))
static void mpi3mr_update_sdev(struct scsi_device *sdev, void *data)
{
	struct mpi3mr_tgt_dev *tgtdev;
	...
	case MPI3_DEVICE_DEVFORM_PCIE:
		...
		if ((tgtdev->dev_spec.pcie_inf.dev_info &
		    MPI3_DEVICE0_PCIE_DEVICE_INFO_TYPE_MASK) ==
		    MPI3_DEVICE0_PCIE_DEVICE_INFO_TYPE_NVME_DEVICE) {
			sdev->request_queue->limits.max_hw_sectors =
				tgtdev->dev_spec.pcie_inf.mdts / 512;
			if (tgtdev->dev_spec.pcie_inf.pgsz == 0)
				sdev->request_queue->limits.virt_boundary_mask =
 ...
 * and reason for diverting IO to firmare.
 *
 * Return: Nothing
 */
	...
	u32 *scsiio_flags)
{
	...
	u8 host_md = 0, opcode = scmd->cmnd[0], xprt = 0;
	u16 sa = (scmd->cmnd[8] << 8) | (scmd->cmnd[9]);
	...
	u8 *resp_code, struct scsi_cmnd *scmd)
{
	...
	if (mrioc->block_on_pcie_err) {
		retval = -1;
		dprint_tm(mrioc, "sending task management failed due to pcie error recovery in progress\n");
		mutex_unlock(&drv_cmd->mutex);
		goto out;
	}
...
static int mpi3mr_eh_target_reset(struct scsi_cmnd *scmd)
{
	...
	struct mpi3mr_stgt_priv_data *stgt_priv_data;
	struct mpi3mr_sdev_priv_data *sdev_priv_data;
	u16 dev_handle, timeout = MPI3MR_RESETTM_TIMEOUT;
	u8 resp_code = 0;
	...
	if (scsi_host_in_recovery(mrioc->shost))
...
static int mpi3mr_eh_abort(struct scsi_cmnd *scmd)
{
	...
	sdev_printk(KERN_INFO, scmd->device, "abort task for scmd(0x%p), "
	    "outstanding for %u ms, timeout %u ms, retries %d, allowed %d\n",
	    scmd, jiffies_to_msecs(jiffies - scmd->jiffies_at_alloc),
	    (rq->timeout / HZ) * 1000, scmd->retries,
	    scmd->allowed);
...
out:
	/*Not to be ported to upstream*/
	if ((retval == FAILED) && (mrioc->poll_completions == false)) {
		mrioc->poll_completions = true;
		ssleep(1);
	}
	sdev_printk(KERN_INFO, scmd->device,
	...
	struct scsi_cmnd *scmd)
{
	...
	dprint_scsi_info(mrioc, "qcmd invoked for scmd(%p) data_len(%d)\n",
	    scmd, scsi_bufflen(scmd));
	...
	/* Avoid error handling escallation when device is removed or blocked */
	if (scmd->device->host->shost_state == SHOST_RECOVERY &&
	    scmd->cmnd[0] == TEST_UNIT_READY &&
	    (stgt_priv_data->dev_removed ||
	    (dev_handle == MPI3MR_INVALID_DEV_HANDLE))) {
		mpi3mr_scsi_build_sense(scmd, 0, UNIT_ATTENTION, 0x29, 0x07);
		SCMD_DONE(scmd);
		goto out;
	}
	...
	if (mrioc->logging_level & MPI3_DEBUG_SG) {
		...
		if (scmd_priv_data->chain_idx >= 0) {
			...
			dprint_dump(chain_req->addr,
			    mrioc->max_sgl_entries *
			    sizeof(struct mpi3_sge_common),
			    "chain_sge");
		}
...
mpi3mr_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	mutex_init(&mrioc->sysfs_tm_mutex);
...
mpi3mr_resume(struct device *dev)
{
	...
	if (r) {
		ioc_err(mrioc, "setup resources failed[%d]\n", r);
		return r;
	}
 ...
 * based on the state passed by the PCI driver this function
 * decides the actions to be recommended back to PCI driver.
 *
 ...
 *
 * For permanent failure state, this funcion will mark the
 ...
 * part of the PCI express Advanced Error Reporting and handling
 *
 * Return: Nothing.
 */
...
static const struct pci_device_id mpi3mr_pci_id_table[] = {
	...
	{
		PCI_DEVICE_SUB(MPI3_MFGPAGE_VENDORID_BROADCOM,
		    MPI3_MFGPAGE_DEVID_SAS5116_MPI_NS, PCI_ANY_ID, PCI_ANY_ID)
	},
	{ 0 }
};
